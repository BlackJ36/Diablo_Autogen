...
user_proxy = autogen.UserProxyAgent(
    name="User",
    system_message='''Admin.Send command to robot.''',
...
corrector = autogen.AssistantAgent(
    ...
    You should receive and analyse the command or question of user.
...
robot = autogen.AssistantAgent(
    name="wheel-legs-bot",
    system_message='''Wheel-legs-bot. 
    You receive the todo motion list from Motion_selector.You must check the todo motion list follow the <Rules>.
If there are no mistakes in todo motion list,you should generate the final_motion from Step of Motion_selector.
...
Motion_selector = autogen.AssistantAgent(
    ...
    Ask wheel-legs-bot to confirm todo motion list if it can execute.
...
"""<Rules>
1.For movement motion,wheel-legs-bot can only move forward or backward.If the movement description in a shape or other direction you should calculate the angle first.
...
Thinking:Step1:<interior angle sum> of square shape is 360 degrees,<number of edge> is 4.
         ...
         Step4:The todolist is:Repeat step3 for 4 times.
         Step5:propose a todo list according to the Thinking
...
Thinking:Step1:I face towards 12 oclock direction now.
         ...
         Step6:propose a todo list according to the Thinking
